var kslExtension = {};
kslExtension.displayItems = function(kslItemsArray)
{
	$liveInfo = $('#liveInfo');
...
//
kslExtension.getFeed = function(url, parser) {
...
//
kslExtension.parseFeed = function(xmlResponse)
{
	if(xmlResponse === null)
	{
		return null;
...
kslExtension.storageHandle = function(storage){
	...
	kslExtension.getFeed('http://www.ksl.com/resources/classifieds/rss_.xml' + category, function(xmlResponse){
		kslExtension.parseFeed(xmlResponse);});
	}
};
$(document).ready(function(){
	//	Gets the category from storage set in the options
	chrome.storage.local.get(
		'selectedCategory', kslExtension.storageHandle);
});